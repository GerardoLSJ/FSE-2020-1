...
//     Comunicación UART en un sistema embebido
...
// Descripción:
//      Lectura de temperatura a través de un GPIO de la ESP32
//      proveniente del sensor LM335 (10mV/K°), por lo que se
//      lee el valor en mV,de tal forma que se hace lo siguiente
//      ** Conversión para tener la temperatura en Kelvin y Centigrados
//      para enviar vía UART del ESP32 a la Computadora.
//      ** Transmisión de la temperatura en voltage vía UART de la
//      ESP32 a la Raspberry.
...
// Pines para la comunicación UART entre la ESP32 y la Raspberry
...
void inicio(void) {
    ...
    // TX y RX UART 2, para comunicar con la Raspberry
...
// Transmisión de datos vía UART de la ESP32 a la Raspberry
int enviaDatosTX(const char* logName, const char* data){
    const int len = strlen(data);
    const int txBytes = uart_write_bytes(UART_NUM_2, data, len);
   
    ESP_LOGI(logName, "Se han transmitido %d bytes a la Raspberry", txBytes);
    
    return txBytes;
}

// Recepción de datos vía UART de la Raspberry a la ESP32
// Función de verificación de datos transmitidos
static void recibeDatosRX(void *arg){
    esp_log_level_set("TAREA RX ** ", ESP_LOG_INFO);
    uint8_t* data = (uint8_t*) malloc(1024+1);
    const int rxBytes = uart_read_bytes(UART_NUM_2, data, 1024, 1000 / portTICK_RATE_MS);
    printf("/////");
    if (rxBytes > 0) {
        data[rxBytes] = 0;
        ESP_LOGI("TAREA RX ** ", "Read %d bytes: '%s'", rxBytes, data);
        ESP_LOG_BUFFER_HEXDUMP("TAREA RX ** ", data, rxBytes, ESP_LOG_INFO);
    }
...
static void uart(void *arg){
    ...
    while (1){
        ...
        while (1) {
            ...
            if (s < 0) {
                ...
                if (FD_ISSET(fd, &rfds)) {
                    ...
                    if (read(fd, &buf, 1) > 0) {
                        ...
                        if (buf == 'i'){
                            tomarTemperatura = 1;
                            ESP_LOGI("Tarea TX UART-PC ** ", "Comienza lectura de temperatura");
                        ...
                        if (buf == 'd'){
                            tomarTemperatura = 0;
                            ESP_LOGI("Tarea TX UART-PC ** ", "Termina lectura de temperatura");
            ...
            if (tomarTemperatura == 1){
                esp_log_level_set("Tarea TX UART-PC ** ", ESP_LOG_INFO);
                ...
                // Cconversión del valor leido de voltaje del sensor (temperatura) a caracter
                itoa(tempAux, tempLeida, 10);
                
                // Transmisión del voltaje leido del sensor a la Raspberry vía UART
                enviaDatosTX("Tarea TX UART-RB ** ", tempLeida);

                // Verificación en la misma ESP32 de lo que se esta transmitiendo
                // recibeDatosRX("Tarea TX UART-RB ** ");
...
void app_main(void){
    ...
    inicio();
    xTaskCreate(uart, "UART_TAREA_TX", 1024*2, NULL, configMAX_PRIORITIES-1, NULL);
